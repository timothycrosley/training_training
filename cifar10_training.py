...
class Trainer:
    ...
    def __init__(self, config: TrainingConfig, device: torch.device):
        ...
        self.experiment_dir = Path(
            f"experiments/{config.experiment_name}_{config.get_hash()}"
        )
    ...
    def setup_model_and_training(self, train_loader: DataLoader):
        ...
        self.model = ResNetCustom(
            num_classes=self.config.num_classes, dropout_rate=self.config.dropout_rate
        ).to(self.device)
...
def evaluate_model(
    ...
    test_acc = 100.0 * np.mean(np.array(all_predictions) == np.array(all_targets))
